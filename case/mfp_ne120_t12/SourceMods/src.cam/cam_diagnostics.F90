...
subroutine diag_init()
  ...
  ! Declare the history fields for which this module contains outfld calls.
   ...
   call addfld ('T600       ','K     ',1,   'A','Temperature at 600 mbar pressure surface',phys_decomp)
   ...
   if ( ideal_phys )then
      ...
      call addfld('QRS200     ', 'K/s     ', 1, 'A', 'Solar heating rate 200hpa', phys_decomp)
      call addfld('QRS850     ', 'K/s     ', 1, 'A', 'Solar heating rate 850hpa', phys_decomp)
      call addfld('QRS500     ', 'K/s     ', 1, 'A', 'Solar heating rate 500hpa', phys_decomp)
   end if
...
!! Boundary layer atmospheric stability, temperature, water vapor diagnostics
   ...
   !!! at 250 mb
    if (hist_fld_active('T250')) then
       call vertinterp(ncol, pcols, pver, state%pmid, 25000._r8, state%t, p_surf_t2)
       call outfld('T250    ', p_surf_t2, pcols, lchnk )
    end if
   !!! at 600 mb
    if (hist_fld_active('T600')) then
       call vertinterp(ncol, pcols, pver, state%pmid, 60000._r8, state%t, p_surf_t2)
       call outfld('T600    ', p_surf_t2, pcols, lchnk )
    end if
