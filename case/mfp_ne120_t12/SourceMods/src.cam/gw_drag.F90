...
!--------------------------------------------------------------------------
  ...
  use gw_common,     only: GWBand,new_GWBand
...
subroutine gw_drag_readnl(nlfile)
  ...
  band_oro = new_GWBand(0, gw_dc, fcrit2, wavelength_mid)
  band_mid = new_GWBand(pgwv, gw_dc, 1.0_r8, wavelength_mid)
  band_long = new_GWBand(pgwv_long, gw_dc_long, 1.0_r8, wavelength_long)
...
subroutine gw_init_beres(file_name, band, desc)
  ...
  write(iulog,*) 'chenyuhu cam_pio_openfile',__FILE__,__LINE__
  ...
  allocate(desc%hd(desc%maxh),stat=stat)
...
subroutine gw_tend(state, sgh, pbuf, dt, ptend, cam_in, flx_heat)
  ...
  use coords_1d,  only: Coords1D,chen_new_Coords1D,chen_Coords1D_finalize
  ...
  !p =chen_new_Coords1D(state%pint(:ncol,:))
  call chen_new_Coords1D(p,state%pint(:ncol,:))
