...
!    integer :: ncid
!if(masterproc)then
!!write(*,*)"restart_dynamics 001"
!end if
    call dyn_init1(file, NLFileName, dyn_in, dyn_out)

!if(masterproc)then
!!write(*,*)"restart_dynamics 002"
!end if
   if (iam .lt. par%nprocs) then
    elem=>dyn_in%elem
   else
...
!if(masterproc)then
!!write(*,*)"restart_dynamics 003"
!end if

    ierr = PIO_Get_Att(File, PIO_GLOBAL, 'ne', fne)
!if(masterproc)then
!!write(*,*)"restart_dynamics 004"
!end if
    ierr = PIO_Get_Att(File, PIO_GLOBAL, 'np', fnp)

!if(masterproc)then
!!write(*,*)"restart_dynamics 005"
!end if
...
!if(masterproc)then
!write(*,*)"restart_dynamics 016"
!end if
    deallocate(ldof)

    ldof => get_restart_decomp(elem, nlev)
!if(masterproc)then
!write(*,*)"restart_dynamics 017"
!end if
    call PIO_InitDecomp(pio_subsystem, pio_double, (/ncols,nlev/),ldof , iodesc3d)
!if(masterproc)then
!write(*,*)"restart_dynamics 018"
!end if
    deallocate(ldof)
...
!if(masterproc)then
!write(*,*)"restart_dynamics 032"
!end if

    tl = timelevel%n0
!if(masterproc)then
!write(*,*)"restart_dynamics 033"
!end if
    call TimeLevel_Qdp(timelevel, qsplit, tlQdp)

!if(masterproc)then
!write(*,*)"restart_dynamics 034"
!end if
    call pio_setframe(File,psdesc, t)
!if(masterproc)then
!write(*,*)"restart_dynamics 035"
!end if
    call pio_read_darray(File, psdesc, iodesc2d, var2d, ierr)

!if(masterproc)then
!write(*,*)"restart_dynamics 036"
!end if
    cnt=0
    do ie=1,nelemd
       do j=1,np
...
!if(masterproc)then
!write(*,*)"restart_dynamics 037"
!end if
    call pio_setframe(File,udesc, t)
!if(masterproc)then
!write(*,*)"restart_dynamics 038"
!end if
    call pio_read_darray(File, udesc, iodesc3d, var3d, ierr)

!if(masterproc)then
!write(*,*)"restart_dynamics 039"
!end if
    cnt=0
    do k=1,nlev
       do ie=1,nelemd
...
!if(masterproc)then
!write(*,*)"restart_dynamics 040"
!end if
    call pio_setframe(File,vdesc, t)
!if(masterproc)then
!write(*,*)"restart_dynamics 041"
!end if
    call pio_read_darray(File, vdesc, iodesc3d, var3d, ierr)
!if(masterproc)then
!write(*,*)"restart_dynamics 042"
!end if
    cnt=0
    do k=1,nlev
       do ie=1,nelemd
...
!if(masterproc)then
!write(*,*)"restart_dynamics 043"
!end if
    call pio_setframe(File,tdesc, t)
!if(masterproc)then
!write(*,*)"restart_dynamics 044"
!end if
    call pio_read_darray(File, tdesc, iodesc3d, var3d, ierr)
!if(masterproc)then
!write(*,*)"restart_dynamics 045"
!end if
    cnt=0
    do k=1,nlev
       do ie=1,nelemd
...
!if(masterproc)then
!write(*,*)"restart_dynamics 046"
!end if
    do q=1,qsize_d
       call pio_setframe(File,qdesc(q), t)
!if(masterproc)then
!write(*,*)"restart_dynamics 047"
!end if
       call pio_read_darray(File, qdesc(q), iodesc3d, var3d, ierr)
!if(masterproc)then
!write(*,*)"restart_dynamics 048"
!end if
       cnt=0
       do k=1,nlev
          do ie=1,nelemd
...
!if(masterproc)then
!write(*,*)"restart_dynamics 049"
!end if
       call pio_setframe(File,qdesc_dp(q), t)
!!if(masterproc)then
!write(*,*)"restart_dynamics 050"
!end if
       call pio_read_darray(File, qdesc_dp(q), iodesc3d, var3d, ierr)
!!if(masterproc)then
!write(*,*)"restart_dynamics 051"
!end if
       cnt=0
       do k=1,nlev
          do ie=1,nelemd
...
!!if(masterproc)then
!write(*,*)"restart_dynamics 052"
!end if
    call dyn_init2(dyn_in)

!!if(masterproc)then
!write(*,*)"restart_dynamics 053"
!end if
   if (iam .lt. par%nprocs) then
   else
    deallocate(elem)
