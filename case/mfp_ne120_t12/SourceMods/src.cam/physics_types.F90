...
#endif
    call t_stopf('local_physics_update')
...
!<<<<<<< HEAD
    call t_stopf('phy_state_alloc')
!=======
!>>>>>>> 7e7db5167710273ab39f2e8ab86847eaecf6d0ad

    call t_startf('phy_copy')
    ncol = state_in%ncol
...
! allocate the individual state components
  ...
  call t_stopf('allocate_endrun')


  call t_startf('equal_inf')
  state%lat(:) = inf
  state%lon(:) = inf
  state%ulat(:) = inf
  state%ulon(:) = inf
