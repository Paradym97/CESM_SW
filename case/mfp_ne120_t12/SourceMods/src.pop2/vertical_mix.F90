...
!!! added-start by wangli
#else
   integer (int_kind) :: iparam(10),i  !!added by wangli
   real (r8) ::        rparam(8)     !!added by wangli
#endif
!!! added-end by wangli
...
!!! added-start by wangli
      ...
      iparam(4)=km
      iparam(5)=kvdc
      iparam(6)=size(VDC,DIM=4)
      iparam(7)=0
      iparam(8)=0
      iparam(9)=nt
      if(partial_bottom_cells) iparam(7)=1
      if(lbottom_heat_flx) iparam(8)=1
      rparam(1)=c0
      rparam(2)=p5
      rparam(3)=zt(k)
      rparam(4)=dzwr(k)
      rparam(5)=zw(k)
      rparam(6)=dzr(k)
      rparam(7)=bottom_heat_flx
      rparam(8)=bottom_heat_flx_depth
    call vdifft_c(iparam,rparam,stf,vdc(:,:,:,:,bid),dzt(:,:,:,bid),told,&
                  vtf(:,:,:,bid),vdtk,kmt(:,:,bid))
#else
!!! added-end by wangli
!
...
!!! added-start 
#ifdef ATH
   integer (int_kind) :: ist,ied,jst,jed
   real(r8):: dzwrk,dzrk
#endif
!!! added-end by wangli
!-----------------------------------------------------------------------
...
!!! added-start 
   ...
   dzwrk = dzwr(k)
   dzrk  = dzr(k)
      call vdiffu_c(nx_block,ny_block,k,km,ist,ied,jst,jed,p5,bottom_drag,dzwrk,dzrk,&
      UOLD(:,:,k),VOLD(:,:,k),&
      VVC(:,:,kvvc,bid),DZU(:,:,k,bid),smf,&
      VUF(:,:,bid),VVF(:,:,bid),VDUK,VDVK,kmu(:,:,bid),c0,partial_bottom_cells)
#else
!!! added-end by wangli
...
!-----------------------------------------------------------------------
   ...
   integer (int_kind) :: &
      ...
      bid                 ! local block address
!!! added-start by wangli
   integer (int_kind) :: mt2_ref,sfc_judge,myrank,ierr,ii
#ifdef NOATH
!!! added-end by wangli
...
!!! added-start by wangli
#ifdef ATH
   mt2_ref=size(VDC,DIM=4)
   sfc_judge=0
   if (sfc_layer_type == sfc_layer_varthick) sfc_judge=1
   call impvmixt_c(nfirst,nlast,nx_block,ny_block,nt,km,mt2_ref,sfc_judge,&
                   grav,p5,c0,aidif,dz,c2dtt,afac_t,&
                   psfc,vdc(:,:,:,:,bid),dzt(:,:,2,bid),tnew,told,kmt(:,:,bid),&
                   partial_bottom_cells,ib,ie,jb,je)
#else
!!! added-end by wangli
...
!-----------------------------------------------------------------------
   ...
   iparam(9) =jb
   iparam(10)=je
   iparam(11)=size(VDC,DIM=4)
   iparam(12)=0
   if (sfc_layer_type == sfc_layer_varthick) iparam(12)=1
   iparam(13)=0
   if (partial_bottom_cells) iparam(13)=1
   iparam(14)=my_task
   rparam(1)=c0
   rparam(2)=p5
   rparam(3)=aidif
   rparam(4)=grav
   call impt_cr_c(iparam,rparam,dz,c2dtt,afac_t,psfc,&
                  vdc(:,:,:,:,bid),dzt(:,:,:,bid),rhs,tnew,kmt(:,:,bid))
#else
...
!!! subroutine impvmixu(UNEW, VNEW, this_block) !!! modified by wangli
 subroutine impvmixu(UNEW, VNEW, UOLD, VOLD, this_block)
!!! uold,vold added by wangli to meger a sum part after the original impvmixu
...
!!! added-start by wangli
   ...
   call impvu_c(iparam,rparam,dz,vvc(:,:,:,bid),unew,vnew,uold,vold,&
                dzu(:,:,:,bid),afac_u,kmu(:,:,bid))
#else
!!! added-end by wangli
...
!!! added-start by wangli
   unew=uold+unew  !!! merge the sum part after the impvmiux process
   vnew=vold+vnew  !!!
#endif
!!! added-end by wangli
