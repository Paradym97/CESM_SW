...
! !USES:
   ...
   use hmix_del4, only: init_del4u, init_del4t, hdiffu_del4, advuhdiffu_del4, hdifft_del4
...
! public set by zengyh 2019-04-08:
...
! public set by zengyh 2019-04-09:
   integer (POP_i4), dimension(nt),public :: &
...
!-----------------------------------------------------------------------
   ...
   enddo
      ...
      call define_tavg_field(tavg_HDIFB_TRACER(1),'HDIFB_T',3, &
                                long_name= 'Temp Horizontal Diffusive Flux across Bottom Face',&
                                units='centimeter degC/s',       &
                                grid_loc='3113',                          &
                                coordinates='TLONG TLAT z_w_bot time' )
      call define_tavg_field(tavg_HDIFB_TRACER(2),'HDIFB_S',3, &
                                long_name= 'Salt Horizontal Diffusive Flux across Bottom Face',&
...
! !INTERFACE:
 ...
 subroutine advuhdiffu(k,WORKX,WORKY,WORKX1,WORKY1,WUK,UUU,VVV,UMIXK,VMIXK,this_block)
...
!      call hdiffu_del2(k, HDUK, HDVK, UMIXK, VMIXK, this_block)
   case (hmix_momentum_type_del4)
      call advuhdiffu_del4(k,WORKX,WORKY,WORKX1,WORKY1,WUK,UUU,VVV,UMIXK,VMIXK,this_block)
!   case (hmix_momentum_type_anis)
...
!   if (my_task == master_task) write(stdout,*) '11111111111' 
!   if (my_task == master_task) write(stdout,*) __FILE__,__LINE__,"tavg_HDIFB_TRACER(1)"
   do n = 1,nt
