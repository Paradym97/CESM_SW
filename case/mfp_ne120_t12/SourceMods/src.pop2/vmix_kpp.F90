...
! !PUBLIC DATA MEMBERS:
   ...
   real (r8), dimension(nx_block,ny_block,km,max_blocks_clinic), &
       public, target ::& 
       UI_VEL,VI_VEL,POP_S2,POP_S2N2,POP_N2,POP_epsilon,WORK_MFP,WORK_GM,U1,U7,n2_2,bckg,&
       diff_ri,test1,test2,test3,test4
   ...
   !real (r8), dimension(nx_block,ny_block,km,7,max_blocks_clinic), &!added by yman
   !    public, target ::&
   !    old_UVEL,old_VVEL
...
!-----------------------------------------------------------------------
   ...
   integer (int_kind) ::   &
      ...
      tavg_Ep,tavg_n2,tavg_n1  !epsilon for MFP by yman
...
!-----------------------------------------------------------------------
   ...
   namelist /vmix_kpp_nml/bckgrnd_vdc1, bckgrnd_vdc2,           &
                          ...
                          linertial, lcvmix, lusemfp,lwithoutSi2
...
!-----------------------------------------------------------------------
   ...
   if (my_task == master_task) then
      ...
      write(stdout,fmt_log ) '  whether use mfp           =', lusemfp
      write(stdout,fmt_log ) '  whether without Si2       =', lwithoutSi2
   ...
   call broadcast_scalar(lusemfp,               master_task)
   call broadcast_scalar(lwithoutSi2,           master_task)
...
!-----------------------------------------------------------------------
!
!  if without Si2 is on, use mfp
!
!-----------------------------------------------------------------------


   if (lwithoutSi2 .and. .not. lusemfp) then
     call exit_POP(sigAbort,'ERROR (init_vmix_kpp) reading vmix_kpp_nml without_Si2 must be with MFP')
   endif
...
!-----------------------------------------------------------------------
   ...
   call define_tavg_field(tavg_n1,'mfp_n1',3,               &
                          long_name='n2',             &
                          units='centimeter^2/s',             &
                          grid_loc='3113',                    &
                          coordinates  ='TLONG TLAT z_w_bot time' )
...
!-----------------------------------------------------------------------
!
!  compute mixing due to internal waves
!
!-----------------------------------------------------------------------

   !bckgrnd_vdc = c0
   !if(lcalmfp) call niwmix(bckgrnd_vdc, DBLOC, this_block)
   if(lcalmfp) call niwmix(Ui,Vi,DBLOC, this_block)
   !bckgrnd_vdc = c0
...
!
!-----------------------------------------------------------------------
   !if (lwithoutSi2 .and. lcalmfp) then
   !   call ri_iwmix(DBLOC, VISC, VDC, UUU, VVV, RHOMIX,&
   !              convect_diff, convect_visc, this_block, Ui, Vi)
   !else
   !   call ri_iwmix(DBLOC, VISC, VDC, UUU, VVV, RHOMIX,&
...
! !INPUT PARAMETERS:
   ...
   real (r8), dimension(nx_block,ny_block,km), optional, intent(in) :: &
      Ui,Vi             !by yman
...
!-----------------------------------------------------------------------
   ...
   real (r8), dimension(nx_block,ny_block) :: &
      ...
      VSHEAR1,           &! (local velocity shear)^2 without Si2
      RI_LOC,            &! local Richardson number 
      RI_LOC1,           &! local Richardson number without Si2
      FRI,               &! function of Ri for shear
      FRI1,              &! function of Ri for shear without Si2
      FRI_I,             &! by yman
      FCON,              &
...
!-----------------------------------------------------------------------
   ...
   wget= (1/sqrt(1+(ULAT(:,:,bid)*radian/phi0)**(-2*nw)))**(c1/c4)
...
!-----------------------------------------------------------------------
      ...
      if (k < km) then
         ...
         if (lwithoutSi2 .and. lcalmfp) then 
           FRI1 = & 
              ((UUU(:,:,k)-UI_VEL(:,:,k,bid)*wget)-(UUU(:,:,k+1)-UI_VEL(:,:,k+1,bid)*wget))**2 + &
              ((VVV(:,:,k)-VI_VEL(:,:,k,bid)*wget)-(VVV(:,:,k+1)-VI_VEL(:,:,k+1,bid)*wget))**2
         ...
         if (partial_bottom_cells) then
            ...
            FRI1 = FRI1/(p5*(DZU(:,:,k  ,bid) + DZU(:,:,k+1,bid)))**2
         endif
         ...
         if (l1Ddyn) then
           ...
           call ugrid_to_tgrid(VSHEAR1,FRI1,bid)
         end if
   ...
   do n = 1,num_v_smooth_Ri
      ...
      do k=1,km
         ...
         do i=1,nx_block
            ...
            RI_LOC1(i,j) = WORK01(i,j,k)
            if (KMT(i,j,bid) >= 3) then
               WORK0(i,j,k) = FRI(i,j) + p5*RI_LOC(i,j) + p25*WORK0(i,j,k+1)
               WORK01(i,j,k) = FRI1(i,j) + p5*RI_LOC1(i,j) + p25*WORK01(i,j,k+1)
            endif
            FRI(i,j) = p25*RI_LOC(i,j)
            FRI1(i,j) = p25*RI_LOC1(i,j)
         enddo
         enddo
      enddo
...
! use DS product point to test, global (i,j) = (5,103)
  ...
  200 format(' Rich  i,j,k DIFF =',3(i3,1x),1pe11.4,1x)
        ...
        if (lrich) then
           FRI    = min((max(WORK0(:,:,k),c0))/Riinfty, c1)
           FRI1   = min((max(WORK01(:,:,k),c0))/Riinfty, c1)
           WORK_ri = rich_mix*(c1 - FRI1*FRI1)**3 !by yman
...
!subroutine niwmix(bckgrnd_vdc, DBLOC, this_block)
subroutine niwmix( Ui, Vi, DBLOC, this_block)
    ! !DESCRIPTION:
    !
       !real (r8), dimension(nx_block,ny_block,km,max_blocks_clinic), intent(inout) :: bckgrnd_vdc
       real (r8), dimension(nx_block,ny_block,km), intent(out) :: &
           Ui,Vi
       real (r8), dimension(nx_block,ny_block,km), intent(in) :: &
          DBLOC             ! buoyancy difference between adjacent levels
       ...
       real (r8), dimension(nx_block,ny_block,km) :: En_MFP,N2_temp
    ...
    do k = 1,km
      if (partial_bottom_cells) then
         if(k < km) then
            N2_temp (:,:,k) =  (DBLOC(:,:,k)+eps)/(p5*(DZT(:,:,k,bid) + DZT(:,:,k+1,bid)))
            where(N2_temp(:,:,k) < N2_lower ) N2_temp(:,:,k) = N2_lower
         else
            N2_temp (:,:,k) = (DBLOC(:,:,k)+eps)/(p5*DZT(:,:,k,bid))
            where(N2_temp(:,:,k) < N2_lower ) N2_temp(:,:,k) = N2_lower
         endif
      else
         N2_temp (:,:,k) = (DBLOC(:,:,k)+eps) / (zgrid(k)-zgrid(k+1))
      endif
      bckgrnd_vdc(:,:,k,bid) = Gamm * En_MFP(:,:,k) / N2_temp(:,:,k)
      bckgrnd_vdc(:,:,k,bid) = min(bckgrnd_vdc(:,:,k,bid),niw_mix_max)
    enddo! k
    n2_2(:,:,:,bid)=N2_temp
    
!    bckgrnd_vdc(:,:,:,bid) = Gamm * En_MFP / N2
    bckg(:,:,:,bid)=bckgrnd_vdc(:,:,:,bid)
        do k= 1, km
            !call accumulate_tavg_field(En_MFP(:,:,k)/1e4_r8,tavg_Ep,bid,k)
            call accumulate_tavg_field(N2_temp(:,:,k),tavg_n1 ,bid,k)
        enddo


end subroutine niwmix
...
!-----------------------------------------------------------------------
   ...
   do k = 1,km
       s(:,:,k) = 1/(1-exp(cmplx(0.0,theta_f)))
       s(:,:,k) = merge(s(:,:,k),zeros,ULAT(:,:,bid)/=0.0)
   enddo
...
subroutine compute_MFP_En(En_MFP,Ui,Vi,DBLOC,this_block)
! !DESCRIPTION:
...
!-----------------------------------------------------------------------  	  
   ...
   wget_2d(:,:,bid)=1/sqrt(1+(TLATD(:,:,bid)/lat0)**(-2*nw))
