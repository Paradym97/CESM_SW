...
!   use state_mod, only: state
   use state_mod  ! state in advt_tavg need state_mod by zengyh 2019-03-11:
...
!  state_range_enforce, state_range_ignore, tmax, tmin, smax, smin, pressz
   ...
   use tavg, only: define_tavg_field, accumulate_tavg_field, accumulate_tavg_now &
       , TAVG_BUF_2D, TAVG_BUF_3D       & ! added by zengyh 2019-03-08.
       , dtavg, zt_150m_levs            &
       , avail_tavg_fields        ! added by zengyh 2019-03-08.   
...
! added by zengyh 2019-03-08:
   use io_tools, only: document
 !!!  added by zengyh 2019-04-08:
   use horizontal_mix, only: tavg_HDIFT, tavg_HDIFS, tavg_HDIFN_TRACER, tavg_HDIFE_TRACER, lsubmesoscale_mixing, hmix_tracer_itype, hdifft
   use hmix_del4, only: lvariable_hmixt, DTN,DTS,DTE,DTW,AHF,ah 
   use depend_mod
...
!-----------------------------------------------------------------------
   ...
   integer (int_kind) :: timer_lp01,timer_lp00
!-----------------------------------------------------------------------
!
!  advection flags
!
...
!   integer, public, save :: init_flag=0
...
!-----------timing_var_def_zengyh----------
   real(8)::start_accum,stop_accum,tot_accum
#endif
...
!-----------------------------------------------------------------------
   ...
   do iblock = 1,nblocks_clinic
      ...
      this_block = get_block(blocks_clinic(iblock),iblock)
#ifndef NOATH
      KXYU(:,:,1,iblock) = (eoshift(HUW(:,:,iblock),dim=1,shift=1) - &
                                 HUW(:,:,iblock))*UAREA_R(:,:,iblock)
      KXYU(:,:,2,iblock) = (eoshift(HUS(:,:,iblock),dim=2,shift=1) - &
...
!-----------------------------------------------------------------------
   ...
   call get_timer(timer_lp01,'ADV_MOMENTUM_lp01', nblocks_clinic, &
                                                   distrb_clinic%nprocs)
   call get_timer(timer_lp00,'ADV_MOMENTUM_lp00', nblocks_clinic, &
                                                   distrb_clinic%nprocs)
...
!-----------------------------------------------------------------------
   ...
   call define_tavg_field(tavg_W5_5,'W5_5',2,                          &
                          long_name='Vertical Velocity on level 5',               &
                          units='centimeter/s', grid_loc='3112',       &
                          coordinates='TLONG TLAT time')

   call define_tavg_field(tavg_W11_11,'W11_11',2,                          &
                          long_name='Vertical Velocity on level 5',               &
                          units='centimeter/s', grid_loc='3112',       &
                          coordinates='TLONG TLAT time')

   call define_tavg_field(tavg_W23_23,'W23_23',2,                          &
                          long_name='Vertical Velocity on level 5',               &
                          units='centimeter/s', grid_loc='3112',       &
                          coordinates='TLONG TLAT time')

   call define_tavg_field(tavg_W34_34,'W34_34',2,                          &
                          long_name='Vertical Velocity on level 5',               &
                          units='centimeter/s', grid_loc='3112',       &
                          coordinates='TLONG TLAT time')

   call define_tavg_field(tavg_W41_41,'W41_41',2,                          &
                          long_name='Vertical Velocity on level 5',               &
                          units='centimeter/s', grid_loc='3112',       &
                          coordinates='TLONG TLAT time')
...
! !INTERFACE:
 ...
 subroutine advthdifft(k,HDTK,LTK,WTK,TMIX,UMIX,VMIX,TRCR,UUU,VVV,this_block)
...
!-----------------------------------------------------------------------
   ...
   integer (int_kind) ::  &
     ...
     ib, ie, jb, je,      &! domain limits for advt
     ibegin,iend,jbegin,jend, &!             for hdifft
...
!!!      D2TK,              &! intermediate Del**2 result
      FC,             &! five point stencil coefficients
!      CC,CN,CS,CE,CW,       &
      HDIFFCFL            ! local hdiff cfl number for diagnostics

! defined by zengyh 2019-04-09: 
   real (r8), dimension(nx_block,ny_block,5) :: &
      CNSEW, UVNSEW

   integer (int_kind), dimension(24) :: &
      intpara

   integer (int_kind) :: array_z

   real (r8) :: dzrk
 ! added by zengyh 2019-03-08:
#if !defined(NOATH_TAVG_ADVT_ZYH)

   integer (int_kind) ::  &
      itmp,ktmp,               &
      bufloc,             &! location of field in tavg buffer
      ndims              ! rank of field (2=2d,3=3d)
   integer (int_kind), dimension(16) ::  &
      tavgintpara
   integer (int_kind), save ::  &
      part_bott_cells,    &! transform logical into int 
      numtavg            ! number of all tavg_ids
   integer (int_kind), dimension(48) ::  &
      tavgids=0,              & ! tavg_id
...
#if defined(POPCHKJN) 
#if defined(TAVG_R8)
   real (r8), dimension(nx_block,ny_block,48) :: &
   ...
   real (r8), dimension(48) :: &
      ...
      minerrtavg3d
#else
   real (r4), dimension(nx_block,ny_block,48) :: &
   ...
   interface
     subroutine m_advt_tavg_sw(TAVG_BUF_2D,TAVG_BUF_3D, &
            VTN,UTE,WTK, &
            TR_E,TR_N,TRCR, &
            DZT,TAREA_R, &
            dtavg,&
            tmax,tmin,smax,smin,pressz,&
            tavgids,tavgflags,tavglocs,tavgdims,tavgmeths, &
            tadvtype,tavgintpara &
          ) bind(C, name='m_advt_tavg_sw')
         integer:: tavgintpara(*),tavgids(*),tavgflags(*),tavglocs(*),tavgdims(*),tavgmeths(*), tadvtype(*)
         real(8),value:: dtavg
#ifdef TAVG_R8
...
#endif
         real(8):: VTN(*),UTE(*),WTK(*),TR_E(*),TR_N(*),TRCR(*),DZT(*),TAREA_R(*), &
                   tmax(*),tmin(*),smax(*),smin(*),pressz(*)
     end subroutine m_advt_tavg_sw
   end interface

#endif
! end_added by zengyh 2019-03-08.
!------------------------------------------------------------------
!
...
! JUST athreaded under following conditions:
   if((.not. luse_lw_lim) .and. luse_centered .and. &
      (hmix_tracer_itype .eq. 2)  .and. lvariable_hmixt .and.&
      (.not. lsubmesoscale_mixing) .and. partial_bottom_cells) then
...
!      call zero_ghost_cells(this_block,UVNSEW(:,:,1))
!      call zero_ghost_cells(this_block,UVNSEW(:,:,2))
!      call zero_ghost_cells(this_block,UVNSEW(:,:,3))
!      call zero_ghost_cells(this_block,UVNSEW(:,:,4))
     ...
     jbegin=this_block%jb-1
     jend  =this_block%je+1
     ibegin=this_block%ib-1
...
!         enddo
   ...
   if(my_task.eq.0) then
     write(*,*) 'ib,ie,jb,je: ', ib,ie,jb,je
     write(*,*) 'ibegin,iend,jbegin,jend: ', ibegin,iend,jbegin,jend
   endif
#endif

!       UVNSEW(1:(ib-1),jb,1) = c0
!       UVNSEW(1:(ib-1),jb,2) = c0
!       UVNSEW(1:(ib-1),jb,3) = c0
!       UVNSEW(1:(ib-1),jb,4) = c0
!       UVNSEW((ie+1):nx_block,je,1) = c0
!       UVNSEW((ie+1):nx_block,je,2) = c0
!       UVNSEW((ie+1):nx_block,je,3) = c0
!       UVNSEW((ie+1):nx_block,je,4) = c0
!
!       CNSEW(1:(ibegin-1),jbegin,1) = c0
!       CNSEW(1:(ibegin-1),jbegin,2) = c0
!       CNSEW(1:(ibegin-1),jbegin,3) = c0
!       CNSEW(1:(ibegin-1),jbegin,4) = c0
!       CNSEW(1:(ibegin-1),jbegin,5) = c0
!       CNSEW((iend+1):nx_block,jend,1) = c0
!       CNSEW((iend+1):nx_block,jend,2) = c0
!       CNSEW((iend+1):nx_block,jend,3) = c0
!       CNSEW((iend+1):nx_block,jend,4) = c0
!       CNSEW((iend+1):nx_block,jend,5) = c0

!    call adhdt_loop11_fun(intpara,UUU,VVV,DYU,DXU,DZU,UVNSEW,&
!          DTN,DTS,DTE,DTW,DZT,CNSEW,KMT)

    call adhdt_loop11_fun(intpara,UUU(:,:,k),VVV(:,:,k),&
          DYU(:,:,bid),DXU(:,:,bid),DZU(:,:,k,bid),UVNSEW,&
          DTN(:,:,bid),DTS(:,:,bid),DTE(:,:,bid),DTW(:,:,bid),&
          DZT(:,:,k,bid),CNSEW,&
          KMT(:,:,bid),WTK,WTKB,TAREA_R(:,:,bid))
...
!      FC = (UVNSEW(:,:,1) - UVNSEW(:,:,2) + UVNSEW(:,:,3) - UVNSEW(:,:,4))*TAREA_R(:,:,bid)
...
!	  if (.not. tr_mask(n)) cycle
       if(tr_mask(n)) then      !tr_mask=T
         intpara(20+n) = 1
       else
         intpara(20+n) = 0
       endif
!!      LTK(:,:,n) = c0
!!    call zero_ghost_cells(this_block, LTK(:,:,n))
!!      HDTK(:,:,n) = c0
!!    call zero_ghost_cells(this_block, HDTK(:,:,n))
...
!     enddo
!       LTK(1:(ib-1),jb,n) = c0
!       LTK((ie+1):nx_block,je,n) = c0
!       HDTK(1:(ib-1),jb,n) = c0
!       HDTK((ie+1):nx_block,je,n) = c0
    enddo
...
!------------------------------------------------------------------
        ...
        intpara(20) = sfc_layer_type
        dzrk = dzr(1)


!!!    do n=1,nt
!       intpara(12) = n     ! n_end
!       intpara(13) = n     ! n_start

       intpara(12) = nt     ! n_end
       intpara(13) = 1     ! n_start

       call adhdt_loop13_fun(intpara,UVNSEW,TRCR,TAREA_R(:,:,bid),DZT,LTK,WTK,WTKB,&
           ah,dzrk,AHF,D2TK,HDTK,TMIX,CNSEW)
...
!               WORK(i,j) = ah*CE(i,j)*(D2TK(i+1,j)-D2TK(i,j))
               WORK(i,j) = ah*CNSEW(i,j,3)*(D2TK(i+1,j,n)-D2TK(i,j,n))
            enddo
            enddo
...
!               WORK(i,j) = ah*CN(i,j)*(D2TK(i,j+1)-D2TK(i,j))
               WORK(i,j) = ah*CNSEW(i,j,1)*(D2TK(i,j+1,n)-D2TK(i,j,n))
            enddo
            enddo
...
!------------------------------------------------------------------
!
!  compute tavg diagnostic  for HDIFFT if requested. 
!
!------------------------------------------------------------------
...
!----------------------------
         ...
         tavgids(1) = tavg_WVEL
         tavgids(2) = tavg_WVEL_2
         tavgids(3) = tavg_WVEL2
         ...
         tavgids(10) = tavg_Q
         tavgids(11) = tavg_PV 
         tavgids(12) = tavg_PVWM
         tavgids(13) = tavg_PVWP
         tavgids(14) = tavg_UPV
         tavgids(15) = tavg_VPV 
         tavgids(16) = tavg_UQ
         tavgids(17) = tavg_VQ
         ...
         itmp = 4
         do n=1,nt
           tavgids(j+(n-1)*itmp+1) = tavg_UE_TRACER(n)    
           tavgids(j+(n-1)*itmp+2) = tavg_VN_TRACER(n)   
           tavgids(j+(n-1)*itmp+3) = tavg_WT_TRACER(n)
           tavgids(j+(n-1)*itmp+4) = tavg_ADV_TRACER(n)
         enddo
...
! step into manycore tavg codes:
         ...
         if(my_task==0 .and. mod(numsteps,64)==0 .and. k<=1) then  
            WRITE (*,*) 'pre_athreaded accum_tavg of advt costs zengyh:',stop_accum-start_accum,'(us);'
         endif
#endif


#ifdef POPTESTJN
         if(my_task==0 .and. numsteps<=4 .and. k<=2) then  
            do i=1,numtavg 
               write(*,*) 'set no. tavgid, bufloc, ndims, method, flags: ', i, tavgids(i),tavglocs(i),tavgdims(i),tavgmeths(i),tavgflags(i)
            enddo
         endif
#endif
         ...
         do i=1,numtavg
           if(tavgdims(i)==3) then
             initavg3d(:,:,i) = TAVG_BUF_3D(:,:,k,bid,tavglocs(i))
           elseif(tavgdims(i)==2) then
             initavg3d(:,:,i) = TAVG_BUF_2D(:,:,bid,tavglocs(i))
           endif
         enddo
#endif


#if defined(POPCHKJN) || defined(NOATH_TAVG_ADVT_ZYH)
! original tavg codes:
...
!         FUE =  p5*UTE*TAREA_R(:,:,bid)/DZT(:,:,k,bid)
         FVN =  p5*UVNSEW(:,:,1)*TAREA_R(:,:,bid)/DZT(:,:,k,bid)
         FUE =  p5*UVNSEW(:,:,3)*TAREA_R(:,:,bid)/DZT(:,:,k,bid)
      else
         FVN =  p5*UVNSEW(:,:,1)*TAREA_R(:,:,bid)
         FUE =  p5*UVNSEW(:,:,3)*TAREA_R(:,:,bid)
      endif

      call accumulate_tavg_field(WTK,tavg_WVEL,bid,k)
      if(k==5)call accumulate_tavg_field(WTK,tavg_W5_5,bid,k)
      if(k==11)call accumulate_tavg_field(WTK,tavg_W11_11,bid,k)
      if(k==23)call accumulate_tavg_field(WTK,tavg_W23_23,bid,k)
      if(k==34)call accumulate_tavg_field(WTK,tavg_W34_34,bid,k)
      if(k==41)call accumulate_tavg_field(WTK,tavg_W41_41,bid,k)
   ...
   if (accumulate_tavg_now(tavg_PD)   .or.  &
     ...
     if(accumulate_tavg_now(tavg_WRHO)) then
...
!----------------------------
         ...
         do i=1,numtavg
           if(tavgdims(i)==3) then
             tmptavg3d(:,:,i) = TAVG_BUF_3D(:,:,k,bid,tavglocs(i))
             TAVG_BUF_3D(:,:,k,bid,tavglocs(i)) = initavg3d(:,:,i)
           elseif(tavgdims(i)==2) then
             tmptavg3d(:,:,i) = TAVG_BUF_2D(:,:,bid,tavglocs(i))
             TAVG_BUF_2D(:,:,bid,tavglocs(i)) = initavg3d(:,:,i)
           endif
         enddo
#endif
...
#if !defined(NOATH_TAVG_ADVT_ZYH)
!  JUST ONLY athread the tavg variables for the CASE of tavg_WVEL, tavg_PD,
!  tavg_UE_TRACER(1:2), tavg_VN_TRACER(1:2).
       ...
       if(state_itype==state_type_mwjf.and.state_range_iopt==state_range_enforce) then
         ...
         tavgintpara(1) = nx_block
         tavgintpara(2) = ny_block
         tavgintpara(3) = km
         tavgintpara(4) = k
         tavgintpara(5) = nblocks_clinic
         tavgintpara(6) = bid
         tavgintpara(7) = numtavg
         tavgintpara(8) = part_bott_cells
         tavgintpara(9) = nt
         tavgintpara(10) = tadvect_centered
         tavgintpara(16) = my_task

 
         call m_advt_tavg_sw(TAVG_BUF_2D,TAVG_BUF_3D, &
           !! VTN,UTE,WTK, &
            UVNSEW(:,:,1),UVNSEW(:,:,3),WTK,&
            TRACER_E,TRACER_N,TRCR(:,:,1,1),&
            DZT(:,:,k,bid),TAREA_R(:,:,bid), &
            dtavg,&
            tmax,tmin,smax,smin,pressz,&
            tavgids(:),tavgflags(:),tavglocs(:),tavgdims(:),tavgmeths(:), &
            tadvect_itype(1:nt),tavgintpara(1:16))

#if defined(POPTIMEJN)
         call getlooptime(stop_accum)
         if(my_task==0 .and. (mod(numsteps,64)==0 .or. numsteps<=128)) then  
            WRITE (*,*) 'athreaded accum_tavg of advt costs zengyh:',stop_accum-start_accum,'(us);'
         endif
#endif

       else
! otherwise, run original codes.
         if (partial_bottom_cells) then
           FVN =  p5*VTN*TAREA_R(:,:,bid)/DZT(:,:,k,bid)
           FUE =  p5*UTE*TAREA_R(:,:,bid)/DZT(:,:,k,bid)
         else
         ...
         if (accumulate_tavg_now(tavg_PD)) then
           call state(k,1,TRCR(:,:,k,1), TRCR(:,:,k,2), this_block, RHOFULL=RHOK1)
           call accumulate_tavg_field(RHOK1,tavg_PD,bid,k)
         endif
...
!           if(tavgflags(i)>0) then
! otherwise, tavgid(i) may be null.
             cstat(i,:) = 0
             if(tavgdims(i)==3) then
               initavg3d(:,:,i) = abs(tmptavg3d(:,:,i) - TAVG_BUF_3D(:,:,k,bid,tavglocs(i)))
             elseif(tavgdims(i)==2) then
               initavg3d(:,:,i) = abs(tmptavg3d(:,:,i) - TAVG_BUF_2D(:,:,bid,tavglocs(i)))
             endif
             tmprtavg3d(:,:,i)= merge(initavg3d(:,:,i)/abs(tmptavg3d(:,:,i)),initavg3d(:,:,i),abs(tmptavg3d(:,:,i))>f0)
             maxerrtavg3d(i) = f0
             maxerrtavg3d(i) = maxval(initavg3d(:,:,i))
             maxrerrtavg3d(i) = maxval(tmprtavg3d(:,:,i))
             minerrtavg3d(i) = f1
             minerrtavg3d(i) = minval(initavg3d(:,:,i))
             errtavg3d(i) = sum(initavg3d(:,:,i)/(nx_block*ny_block))
             rerrtavg3d(i)= sum(tmprtavg3d(:,:,i)/(nx_block*ny_block))
             ...
             do j=1,ny_block
             do itmp=1,nx_block
               if(tmprtavg3d(itmp,j,i)>=1.0e-2) then
                  cstat(i,1) = cstat(i,1) + 1
               else if(tmprtavg3d(itmp,j,i)>=1.0e-4) then
                  cstat(i,2) = cstat(i,2) + 1
               else if(tmprtavg3d(itmp,j,i)>=1.0e-6) then
                  cstat(i,3) = cstat(i,3) + 1
               else if(tmprtavg3d(itmp,j,i)>=1.0e-8) then
             ...
             if(my_task==0) then
               ...
               do j=1,ny_block
               do itmp=1,nx_block
                 if(tmprtavg3d(itmp,j,i)>1.0e-4) then
                   write(*,*) 'e-4(i,j) ',i, tavgids(i), ', k: ', k, ', j: ', j, ', i: ', itmp, '; m_value: ', tmptavg3d(itmp,j,i), ', s_value: ', initavg3d(itmp,j,i), ', rel_error: ', tmprtavg3d(itmp,j,i)
                 else if(tmprtavg3d(itmp,j,i)>1.0e-6) then
                   write(*,*) 'e-6(i,j) ',i, tavgids(i), ', k: ', k, ', j: ', j, ', i: ', itmp, '; m_value: ', tmptavg3d(itmp,j,i), ', s_value: ', initavg3d(itmp,j,i), ', rel_error: ',tmprtavg3d(itmp,j,i)
                 else if(tmprtavg3d(itmp,j,i)>1.0e-8) then
                   write(*,*) 'e-8(i,j) ',i, tavgids(i), ', k: ', k, ', j: ', j, ', i: ', itmp, '; m_value: ', tmptavg3d(itmp,j,i), ', s_value: ', initavg3d(itmp,j,i), ', rel_error: ',tmprtavg3d(itmp,j,i)
                 endif
               enddo
               enddo
             endif
...
#if defined(POPTIMEJN)
!      'original accum_tavg of advt costs about 1400(us), except for state calls.'
       numsteps = numsteps + 1
#endif
...
!   call cfl_advect(k,bid,UTE,UTW,VTN,VTS,WTK,WTKB,this_block)
   call cfl_advect(k,bid,UVNSEW(:,:,3),UVNSEW(:,:,4),UVNSEW(:,:,1),UVNSEW(:,:,2),WTK,WTKB,this_block)
...
!-----------------------------------------------------------------------
   ...
   else      ! use original codes:
     call hdifft(k, HDTK, TMIX, UMIX, VMIX, this_block)
     call advt(k,LTK,WTK,TMIX,TRCR,UUU,VVV,this_block)
...
!-----------------------------------------------------------------------
   ...
   if (partial_bottom_cells) then
      ...
      do j=jbeg-1,jend
      do i=ibeg-1,iend
         udud(i,j)=p5*uuu(i,j,k)*dyu(i,j,bid)*dzu(i,j,k,bid)
         vdvd(i,j)=p5*vvv(i,j,k)*dxu(i,j,bid)*dzu(i,j,k,bid)
      enddo
      enddo
...
!                        UUU(i  ,j-1,k)*DYU(i  ,j-1  ,bid)*  &
!                                       DZU(i  ,j-1,k,bid))
         ute(i,j)=udud(i,j)+udud(i,j-1)
         utw(i,j)=udud(i-1,j)+udud(i-1,j-1)
...
!                        VVV(i-1,j  ,k)*DXU(i-1,j    ,bid)*  &
!                                       DZU(i-1,j  ,k,bid))
          vtn(i,j)=vdvd(i-1,j)+vdvd(i,j)
          vts(i,j)=vdvd(i-1,j-1)+vdvd(i,j-1)
