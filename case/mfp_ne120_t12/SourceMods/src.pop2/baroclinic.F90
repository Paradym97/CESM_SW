...
!-----------------------------------------------------------------------
   ...
   integer (int_kind) :: &
      ...
      tavg_TEND_UVEL,         &! wanghong
      tavg_TEND_VVEL,         &! wanghong
      tavg_GRADX,         &! wanghong
      tavg_GRADY,         &! wanghong
      tavg_ADVU,         &! wanghong
      tavg_ADVV,         &! wanghong
      tavg_HDIFFU,         &! wanghong
      tavg_HDIFFV,         &! wanghong
      tavg_VDIFFU,         &! wanghong
      tavg_VDIFFV,         &! wanghong
      tavg_T5_5,         &! wanghong
      tavg_S5_5,         &! wanghong
      tavg_T34_34,         &! wanghong
      tavg_S34_34,         &! wanghong
      tavg_T41_41,         &! wanghong
      tavg_S41_41,         &! wanghong
      tavg_T23_23,         &! wanghong
      tavg_S23_23,         &! wanghong
      tavg_T11_11,         &! wanghong
      tavg_S11_11,         &! wanghong
...
!! added by wanghong 20210122
   ...
   call define_tavg_field(tavg_ADVU,'ADVU',3,                          &
                          long_name='advection in x',           &
                          units='cm/s^2', grid_loc='3221',               &
                          coordinates='TLONG TLAT z_t time')

   call define_tavg_field(tavg_ADVV,'ADVV',3,                          &
                          long_name='advection in y',           &
                          units='cm/s^2', grid_loc='3221',               &
                          coordinates='TLONG TLAT z_t time')

   call define_tavg_field(tavg_GRADX,'GRADX',3,                          &
                          long_name='X components of the pressure gradient',           &
                          units='cm/s^2', grid_loc='3221',               &
                          coordinates='TLONG TLAT z_t time')

   call define_tavg_field(tavg_GRADY,'GRADY',3,                          &
                          long_name='Y components of the pressure gradient',           &
                          units='cm/s^2', grid_loc='3221',               &
                          coordinates='TLONG TLAT z_t time')

   call define_tavg_field(tavg_TEND_UVEL,'TEND_UVEL',3,                          &
                          long_name='Tendency of UVEL',           &
                          units='cm/s^2', grid_loc='3221',               &
                          coordinates='TLONG TLAT z_t time')

   call define_tavg_field(tavg_TEND_VVEL,'TEND_VVEL',3,                          &
                          long_name='Tendency of VVEL',           &
                          units='cm/s^2', grid_loc='3221',               &
                          coordinates='TLONG TLAT z_t time')
!!!end added by wanghong 20210122
   ...
   call define_tavg_field(tavg_T5_5,'T5_5',2,                          &
                          long_name='Potential Temperature lvls 5(50m)',  &
                          units='degC', grid_loc='2111')

   call define_tavg_field(tavg_T11_11,'T11_11',2,                          &
                          long_name='Potential Temperature lvls 11(100m)',  &
                          units='degC', grid_loc='2111')

   call define_tavg_field(tavg_T23_23,'T23_23',2,                          &
                          long_name='Potential Temperature lvls 23(200m)',  &
                          units='degC', grid_loc='2111')

   call define_tavg_field(tavg_T34_34,'T34_34',2,                          &
                          long_name='Potential Temperature lvls 34(500m)',  &
                          units='degC', grid_loc='2111')

   call define_tavg_field(tavg_T41_41,'T41_41',2,                          &
                          long_name='Potential Temperature lvls 41(1000m)',  &
                          units='degC', grid_loc='2111')

   call define_tavg_field(tavg_S5_5,'S5_5',2,                          &
                          long_name='Salinity lvls 5(50m)',  &
                          units='gram/kilogram', grid_loc='2111')

   call define_tavg_field(tavg_S11_11,'S11_11',2,                          &
                          long_name='Salinity lvls 11(100m)',  &
                          units='gram/kilogram', grid_loc='2111')

   call define_tavg_field(tavg_S23_23,'S23_23',2,                          &
                          long_name='Salinity lvls 23(200m)',  &
                          units='gram/kilogram', grid_loc='2111')

   call define_tavg_field(tavg_S34_34,'S34_34',2,                          &
                          long_name='Salinity lvls 34(500m)',  &
                          units='gram/kilogram', grid_loc='2111')

   call define_tavg_field(tavg_S41_41,'S41_41',2,                          &
                          long_name='Salinity lvls 41(1000m)',  &
                          units='gram/kilogram', grid_loc='2111')
...
!-----------------------------------------------------------------------
   ...
   real (r8), dimension(nx_block,ny_block) :: & 
      ...
      WORK1,WORK2,WORK,        &! local work space
...
! added by zengyh 2019-02-15:
   ...
   integer (int_kind), parameter :: numtavg=39
   integer (int_kind) ::  &
      itmp,               &
   ...
   integer (int_kind), save ::  &
     ...
     ! numtavg=39            ! number of all tavg_ids
   integer (int_kind), dimension(numtavg), save ::  &
      tavgids=0,              & ! tavg_id
      tavglocs=-1,            & ! bufloc of tavg_id
      tavgdims=0,             & ! ndims of tavg_id
      tavgmeths=-1               ! tavg method of tavg_id
   integer (int_kind), dimension(numtavg*km) ::  &
...
#if defined(POPCHKJN) 
#if defined(TAVG_R8)
   real (r8), dimension(nx_block,ny_block,km,numtavg) :: &
      initavg3d=c0, tmptavg3d=c0, tmprtavg3d=c0
   real (r8), dimension(numtavg) :: &
      errtavg3d, rerrtavg3d,             &
      maxerrtavg3d, maxrerrtavg3d,       &
      minerrtavg3d
#else
   real (r4), dimension(nx_block,ny_block,km,numtavg) :: &
      initavg3d=f0, tmptavg3d=f0, tmprtavg3d=f0
   real (r4), dimension(numtavg) :: &
      errtavg3d, rerrtavg3d,             &
      maxerrtavg3d, maxrerrtavg3d,       &
      minerrtavg3d
#endif  
   integer (int_kind) ::     &
      mykm = -1
   integer (int_kind), dimension(numtavg,5) ::&
      cstat
#endif  
...
!----------------------------
	 ...
	 tavgids(37) = tavg_SSS
	 tavgids(38) = tavg_U2_2
	 tavgids(39) = tavg_V2_2
!!	 tavgids(40) = tavg_ADVU
!!	 tavgids(41) = tavg_ADVV
!!	 tavgids(42) = tavg_GRADX
!!	 tavgids(43) = tavg_GRADY
!!	 tavgids(44) = tavg_HDIFFU
!!	 tavgids(45) = tavg_HDIFFV
!!	 tavgids(46) = tavg_VDIFFU
!!	 tavgids(47) = tavg_VDIFFV
!!	 tavgids(48) = tavg_TEND_UVEL
!!	 tavgids(49) = tavg_TEND_VVEL
...
! step into manycore tavg codes:
         ...
         if(k>2) then
            tavgflags((38-1)*km+k) = 0
            tavgflags((39-1)*km+k) = 0
         endif
         if(k<2) then
            tavgflags((38-1)*km+k) = 0
            tavgflags((39-1)*km+k) = 0
 ...
 !!!added by wanghong
	 ...
	 if(k .eq. 1) then
            WORK = c1 / c2dtt(k) *    &
              ((c1 + PSURF(:,:,newtime,iblock)/grav/dz(k))*UVEL(:,:,k,newtime,iblock) - &
               (c1 + PSURF(:,:,oldtime,iblock)/grav/dz(k))*UVEL(:,:,k,oldtime,iblock))
            call accumulate_tavg_field(WORK, tavg_TEND_UVEL, iblock, k)
	 else
               WORK = (UVEL(:,:,k,newtime,iblock) -             &
                       UVEL(:,:,k,oldtime,iblock)) / c2dtt(k)
               call accumulate_tavg_field(WORK, tavg_TEND_UVEL, iblock, k)
         endif
      endif
      if (mix_pass /= 1) then
         if(k .eq. 1)then
            WORK = c1 / c2dtt(k) *    &
              ((c1 + PSURF(:,:,newtime,iblock)/grav/dz(k))*VVEL(:,:,k,newtime,iblock) - &
               (c1 + PSURF(:,:,oldtime,iblock)/grav/dz(k))*VVEL(:,:,k,oldtime,iblock))
            call accumulate_tavg_field(WORK, tavg_TEND_VVEL, iblock, k)
         else
               WORK = (VVEL(:,:,k,newtime,iblock) -             &
                       VVEL(:,:,k,oldtime,iblock)) / c2dtt(k)
               call accumulate_tavg_field(WORK, tavg_TEND_VVEL, iblock, k)
         endif
      endif
...
!----------------------------
         ...
         if (k == 5)  then
            call accumulate_tavg_field(TRACER(:,:,5,1,curtime,iblock), &
                                       tavg_T5_5,iblock,1)

            call accumulate_tavg_field(TRACER(:,:,5,2,curtime,iblock), &
                                       tavg_S5_5,iblock,1)
         endif

         if (k == 11)  then
            call accumulate_tavg_field(TRACER(:,:,11,1,curtime,iblock), &
                                       tavg_T11_11,iblock,1)

            call accumulate_tavg_field(TRACER(:,:,11,2,curtime,iblock), &
                                       tavg_S11_11,iblock,1)
         endif

         if (k == 23)  then
            call accumulate_tavg_field(TRACER(:,:,23,1,curtime,iblock), &
                                       tavg_T23_23,iblock,1)

            call accumulate_tavg_field(TRACER(:,:,23,2,curtime,iblock), &
                                       tavg_S23_23,iblock,1)
         endif

         if (k == 34)  then
            call accumulate_tavg_field(TRACER(:,:,34,1,curtime,iblock), &
                                       tavg_T34_34,iblock,1)

            call accumulate_tavg_field(TRACER(:,:,34,2,curtime,iblock), &
                                       tavg_S34_34,iblock,1)
         endif

         if (k == 41)  then
            call accumulate_tavg_field(TRACER(:,:,41,1,curtime,iblock), &
                                       tavg_T41_41,iblock,1)

            call accumulate_tavg_field(TRACER(:,:,41,2,curtime,iblock), &
                                       tavg_S41_41,iblock,1)
         endif
...
!
   if (.not.l1Ddyn) then
    ...
    call accumulate_tavg_field(WORKX1,tavg_HDIFFU,bid,k)
    call accumulate_tavg_field(WORKY1,tavg_HDIFFV,bid,k) 
#endif
...
! original calls:
    ...
    call accumulate_tavg_field(-WORKY,tavg_ADVV,bid,k) 
    call accumulate_tavg_field(WORKX1,tavg_HDIFFU,bid,k)
    call accumulate_tavg_field(WORKX2,tavg_HDIFFV,bid,k) 
#endif
...
!-----------------------------------------------------------------------
     ...
     WORKX  = SUMX(:,:,bid)
     WORKY  = SUMY(:,:,bid)
     if(k.eq.5) write(stdout,*)"WORKX= ",WORKX
     if(k.eq.5) write(stdout,*)"RHOKX= ",RHOKX
     if(mytask.eq.master_task) write(stdout,*) bouss
