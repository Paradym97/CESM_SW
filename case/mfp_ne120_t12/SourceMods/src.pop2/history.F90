...
!-----------------------------------------------------------------------
   ...
   integer (int_kind) ::   &! history field ids
      ...
      hist_id_n2,hist_id_epsilon,hist_id_kvniw,&
      ...
      hist_id_ri_s2,hist_id_ri_s2_2,hist_id_ri_ri,&
      hist_id_wget,hist_id_n1,hist_id_bck,hist_id_s2,hist_id_s2n2,&
      hist_id_test1,hist_id_test2,hist_id_test3,hist_id_test4
!EOC
!***********************************************************************
      ...
      do nfield = 1,num_avail_hist_fields  ! check all available fields
         ...
         if (avail_hist_fields(nfield)%requested) then 
            ...
            do iblock=1,nblocks_clinic
               ...
               else if (nfield == hist_id_n1) then !added by man
                  WORK3D(:,:,:,iblock) = n2_2(:,:,:,iblock)
               else if (nfield == hist_id_u1) then !added by man
                  WORK3D(:,:,:,iblock) = U1(:,:,:,iblock)
               else if (nfield == hist_id_u7) then !added by man
                  WORK3D(:,:,:,iblock) = U7(:,:,:,iblock)
               else if (nfield == hist_id_test1) then !added by man
                  WORK3D(:,:,:,iblock) = test1(:,:,:,iblock)
               else if (nfield == hist_id_test2) then !added by man
                  WORK3D(:,:,:,iblock) = test2(:,:,:,iblock)
               else if (nfield == hist_id_test3) then !added by man
                  WORK3D(:,:,:,iblock) = test3(:,:,:,iblock)
               else if (nfield == hist_id_test4) then !added by man
                  WORK3D(:,:,:,iblock) = test4(:,:,:,iblock)
               else if (nfield == hist_id_bck) then !added by man
                  WORK3D(:,:,:,iblock) = bckg(:,:,:,iblock)
               ...
               else if (nfield == hist_id_wget) then !added by man
                  WORK2D(:,:,iblock) = wget_2d(:,:,iblock)
               endif
            end do !block loop
            !$OMP END PARALLEL DO
...
!-----------------------------------------------------------------------
   ...
   if (lhistory_on) then
      ...
      call define_hist_field(hist_id_bck, 'bck', 3,         &
                             long_name = 'bck', &
                             units     = 's^-2',                  &
                             grid_loc  = '1111')
      ...
      call define_hist_field(hist_id_test1, 'test1', 3,         &
                             long_name = 'u1', &
                             units     = ' ',                  &
                             grid_loc  = '1111')

      call define_hist_field(hist_id_test2, 'test2', 3,         &
                             long_name = 'u1', &
                             units     = ' ',                  &
                             grid_loc  = '1111')

      call define_hist_field(hist_id_test3, 'test3', 3,         &
                             long_name = 'u1', &
                             units     = ' ',                  &
                             grid_loc  = '1111')

      call define_hist_field(hist_id_test4, 'test4', 3,         &
                             long_name = 'u1', &
                             units     = ' ',                  &
                             grid_loc  = '1111')
