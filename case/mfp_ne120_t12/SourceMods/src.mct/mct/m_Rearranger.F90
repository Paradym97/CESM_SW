...
!-----------------------------------------------------------------------
   ...
   integer, parameter :: RadixRange = 256
   integer, parameter :: RadixBit = 8
   integer :: RadixPool(0:RadixRange-1)
   integer :: tmplist0(max_nprocs)
   integer :: tmplist1(max_nprocs)
   integer :: SList(max_nprocs)
   integer :: RList(max_nprocs)
...
!::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
  ...
  RadixPool(:) = 0
  do proc = 1, SendRout%nprocs
    u = iand(SendRout%pe_list(proc), RadixRange-1)
    RadixPool(u) = RadixPool(u) + 1
  end do
  do i = 1, RadixRange - 1
    RadixPool(i) = RadixPool(i) + RadixPool(i-1) 
  end do
  do proc = SendRout%nprocs, 1, -1
    u = iand(SendRout%pe_list(proc), RadixRange-1)
    tmplist0(RadixPool(u)) = proc
    RadixPool(u) = RadixPool(u) - 1
  end do
  RadixPool(:) = 0
  do proc = 1, SendRout%nprocs
    u = shiftr(SendRout%pe_list(tmplist0(proc)), RadixBit)
    RadixPool(u) = RadixPool(u) + 1
  end do
  do i = 1, RadixRange - 1
    RadixPool(i) =  RadixPool(i) + RadixPool(i-1)
  ...
  do proc = SendRout%nprocs, 1, -1
    u = shiftr(SendRout%pe_list(tmplist0(proc)), RadixBit)
    tmplist1(RadixPool(u)) = tmplist0(proc)
    RadixPool(u) = RadixPool(u) - 1
  end do
  u = 0
  do proc = 1,SendRout%nprocs
    if (SendRout%pe_list(tmplist1(proc)) > myPid) then
        u = u + 1
        SList(u) = tmplist1(proc)
    end if
  end do
  do proc = 1, SendRout%nprocs - u
    SList(u+proc) = tmplist1(proc)
  end do
  ...
  RadixPool(:) = 0
  do proc = 1, RecvRout%nprocs
    u = iand(RecvRout%pe_list(proc), RadixRange-1)
    RadixPool(u) = RadixPool(u) + 1
  end do
  do i = 1, RadixRange - 1
    RadixPool(i) = RadixPool(i) + RadixPool(i-1) 
  end do
  do proc = RecvRout%nprocs, 1, -1
    u = iand(RecvRout%pe_list(proc), RadixRange-1)
    tmplist0(RadixPool(u)) = proc
    RadixPool(u) = RadixPool(u) - 1
  end do
  RadixPool(:) = 0
  do proc = 1, RecvRout%nprocs
    u = shiftr(RecvRout%pe_list(tmplist0(proc)), RadixBit)
    RadixPool(u) = RadixPool(u) + 1
  end do
  do i = 1, RadixRange - 1
    RadixPool(i) =  RadixPool(i) + RadixPool(i-1)
  ...
  u = 0
  do proc = 1, RecvRout%nprocs
    if (RecvRout%pe_list(tmplist1(proc)) > myPid) then
        u = u + 1
        RList(u) = tmplist1(proc)
    end if
  end do
  do proc = 1, RecvRout%nprocs - u
    RList(u+proc) = tmplist1(proc)
  end do
  ! if (mypid == 1259) then
  ! do proc = 1, RecvRout%nprocs
  !   write(*,*) 'Recv', RList(proc), RecvRout%pe_list(proc)
  ! end do
  ! end if
...
if (usealltoall) then
  ...
  !  if (proc .ne. -1) then
  !   u = u + 1
  !   if (proc .ne. RList(u)) write(*,*) 'R', mypid, ':', proc, RList(u)
  !  end if
  ! end do
  ! if (u .ne. RecvRout%nprocs) write(*,*) 'R', mypid, ':', proc, RList(u)

  ! do pe_shift = 1,max_pe
  !  proc = RecvList(mod(myPid+pe_shift,max_pe))
  do i = 1, RecvRout%nprocs
   proc = RList(i)
    ! if (proc .ne. -1) then
...
!::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
  ...
  !  if (proc .ne. -1) then
  !   if (proc .ne. SList(u)) write(*,*) 'R', mypid, ':', proc, SList(u)
  !   u = u - 1
  !  end if
  ! end do
  ! if (u .ne. 0) write(*,*) 'R', mypid, ':', proc, SList(u)

  ! do pe_shift = max_pe,1,-1
  !  proc = SendList(mod(myPid+pe_shift,max_pe))
  do i = SendRout%nprocs, 1, -1
   proc = SList(i)
    ! if (proc .ne. -1) then
